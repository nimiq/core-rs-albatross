...
use crate::messages::{RequestBatchSet, RequestBlock, RequestBlockHashes, RequestHistoryChunk};
use crate::Consensus;
...
impl<N: Network> Consensus<N> {
    pub(super) fn init_network_requests(network: &Arc<N>, blockchain: &Arc<Blockchain>) {
        ...
        tokio::spawn(async move {
            while let Some((msg, peer)) = stream.next().await {
                trace!("[REQUEST_BLOCK] for block hash {} received from {:?}", msg.hash, peer.id());
