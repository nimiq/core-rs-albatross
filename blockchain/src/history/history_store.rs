...
impl HistoryInterface for HistoryStore {
    ...
    /// - `None`: If the block is not a macro block or if there are no transactions or if the keccak256 proof support is disabled.
    ///
    /// # Performance
    ///
    ...
    /// - `None`: If the block is not a macro block, index is invalid, or no transactions exist, or the keccak256 proof support is disabled.
    ///
    /// # Performance
    ///
    ...
    ///
    /// // Verify the proof using sorted verification (OpenZeppelin compatible)
