...
impl ValidityStore {
    ...
    /// (right after genesis or a fresh sync). An emptied store is harmless — `first_bn`/`last_bn`
    /// fall back to 0 and `update_validity_store` repopulates it with the next block.
