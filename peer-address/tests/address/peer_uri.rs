...
fn test_parse_uri_dumb() {
    ...
    assert_eq!(
        uri.peer_id(),
        Some(String::from("2b3f0f59334ef71ee7869b451139587f")).as_ref()
    );
}
...
fn test_parse_uri_ws_port_peerid() {
    let uri =
        PeerUri::from_str("ws://seed-20.nimiq.com:8443/2b3f0f59334ef71ee7869b451139587f").unwrap();
    assert_eq!(uri.protocol(), Protocol::Ws);
    assert_eq!(
        uri.hostname(),
        Some(String::from("seed-20.nimiq.com")).as_ref()
    );
...
fn test_parse_uri_ws() {
    let uri =
        PeerUri::from_str("wss://seed-20.nimiq.com/2b3f0f59334ef71ee7869b451139587f").unwrap();
