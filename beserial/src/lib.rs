...
// Arrays
// TODO Turn this into a macro and add support for more array sizes.

impl<T: Serialize> Serialize for [T; 16] {
    fn serialize<W: WriteBytesExt>(&self, writer: &mut W) -> Result<usize, SerializingError> {
        let mut size = 0;
        for item in self {
            size += Serialize::serialize(item, writer)?;
        }
        Ok(size)
    }
    ...
    fn serialized_size(&self) -> usize {
        let mut size = 0;
        for item in self {
            size += Serialize::serialized_size(item);
        }
        size
    }
}
