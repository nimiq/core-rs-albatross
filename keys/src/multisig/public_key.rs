...
use crate::{multisig::hash_public_keys, EdDSAPublicKey};
...
impl DelinearizedPublicKey {
    ...
    fn new(public_key: EdDSAPublicKey, hash: &[u8; 64]) -> Self {
    ...
    pub fn delinearize(public_keys: &[EdDSAPublicKey]) -> Vec<Self> {
    ...
    /// Delinearizaion prevents rogue key attacks.
    pub fn sum_delinearized(public_keys: &[EdDSAPublicKey]) -> EdDSAPublicKey {
...
impl From<DelinearizedPublicKey> for EdDSAPublicKey {
    fn from(dpk: DelinearizedPublicKey) -> Self {
        EdDSAPublicKey::from(dpk.0.compress().to_bytes())
    }
}
