...
macro_rules! from_hex_vec {
    ($vec: expr, $len: expr, $call: path) => {{
        $vec.iter().map(|item| from_hex!(item, $len, $call)).collect()
    }};
    ($vec: expr, $len: expr) => {{
...
impl TestVector {
    fn from_str(v: &StrTestVector) -> TestVector {
        ...
        let delinearized_pub_keys: Vec<EdwardsPoint> = from_hex_vec!(v.delinearized_pub_keys, PublicKey::SIZE)
...
fn it_can_create_partial_signatures() {
    for vector in VECTORS.iter() {
        ...
        for i in 0..test.priv_keys.len() {
            ...
            let (partial_signature, agg_public_key, agg_commitment) =
                key_pair.partial_sign(&public_keys, &test.secrets[i], &test.commitments, &test.message.as_slice());
...
fn it_sign_and_verify_multisigs() {
    for vector in VECTORS.iter() {
        ...
        for i in 0..test.priv_keys.len() {
            ...
            let (partial_signature, agg_public_key, agg_commitment) =
                key_pair.partial_sign(&test.pub_keys, cp.random_secret(), &test.commitments, &test.message.as_slice());
