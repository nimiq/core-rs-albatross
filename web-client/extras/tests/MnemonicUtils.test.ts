...
import { BufferUtils, Entropy, MnemonicUtils } from '../lib';

describe('MnemonicUtils', () => {
    let originalEntropySize: number;
    ...
    it('correctly computes mnemonic type', () => {
        expect(
            MnemonicUtils.getMnemonicType(
                'void come effort suffer camp survey warrior heavy shoot primary clutch crush open amazing screen patrol group space point ten exist slush involve unfold',
            ),
        ).toBe(MnemonicUtils.MnemonicType.BIP39);
        expect(
            MnemonicUtils.getMnemonicType(
        ...
        ).toBe(MnemonicUtils.MnemonicType.LEGACY);
        // May be both:
        expect(
            MnemonicUtils.getMnemonicType(
        ...
        ).toBe(MnemonicUtils.MnemonicType.UNKNOWN);
        // Invalid checksum:
        expect(() =>
            MnemonicUtils.getMnemonicType(
                'abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon art abandon abandon abandon abandon abandon abandon art',
            )
        ).toThrowError('Invalid checksum');
    });

    it('correctly detects collisions', () => {
        const entropy = MnemonicUtils.mnemonicToEntropy(
            'enforce valid stock pool lonely label awesome olive torch pledge enhance deny you rude inspire ill letter pulse pact light timber enable elbow guide',
        );
        expect(MnemonicUtils.isCollidingChecksum(entropy)).toBe(true);
    });

    it.skip('correctly computes legacy mnemonics', () => {
        ...
        const vectors = [
            ...
            },
            {
                ...
                mnemonic:
                    'warm baby fine emerge giggle agree measure chair now vanish welcome menu spray similar lumber',
            },
            {
                entropy: new Uint8Array([
                    161,
                ...
                mnemonic:
                    'patient expire material rich fashion legend sunny hobby brown inhale jaguar doctor tank primary roast garage friend celery',
            },
            {
                entropy: new Uint8Array([
                    40,
                ...
                mnemonic:
                    'churn vendor tornado shift maid often episode snake parrot grunt harsh armor peace verify student elephant injury frame bird cargo canyon',
            },
            {
                entropy: new Uint8Array([
                    169,
                ...
                mnemonic:
                    'prefer foil call dove gym shop style blur used chuckle rain destroy person leader affair pretty weekend resemble ostrich ugly token glass nature visa',
            },
        ];
        ...
        for (let i = 0; i < vectors.length; i++) {
            ...
            // @ts-expect-error Method not implemented
