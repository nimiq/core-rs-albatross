...
use nimiq_nano_sync::primitives::{merkle_tree_construct, merkle_tree_prove, merkle_tree_verify, pedersen_generators, pedersen_hash};
...
impl ConstraintSynthesizer<MNT4Fr> for VerifyCircuit {
    fn generate_constraints<CS: ConstraintSystem<MNT4Fr>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        ...
        let nodes_var = Vec::<G1Gadget>::alloc(cs.ns(|| "alloc nodes"), || Ok(self.nodes.as_ref()))?;
        ...
        let generators_var = Vec::<G1Gadget>::alloc_constant(cs.ns(|| "alloc pedersen_generators"), pedersen_generators(4))?;

        // Verify Merkle proof.
        MerkleTreeGadget::verify(cs.ns(|| "verify merkle proof"), &leaf_var, &nodes_var, &path_var, &root_var, &generators_var)
    }
}
...
fn construct_works() {
    ...
    for leaf in leaves {
        for i in 0..leaf.len() {
            bits_var.push(Boolean::alloc(cs.ns(|| format!("allocating input bit {} {}", j, i)), || Ok(&leaf[i])).unwrap());
        }
    ...
    let gadget_out = MerkleTreeGadget::construct(cs.ns(|| "evaluate pedersen gadget"), &leaves_var, &c_generators).unwrap();
...
fn verify_works() {
    ...
    assert!(merkle_tree_verify(leaf.clone(), nodes.clone(), path.clone(), root.clone(),));
