...
fn prove(
    ...
    prove_macro_block_wrapper(rng, &initial_pks, &final_pks, block.block_number)?;
    ...
    let proof = prove_merger_wrapper(rng, &initial_pks, &final_pks, block.block_number)?;
...
fn prove_merger_wrapper<R: CryptoRng + Rng>(
    ...
    // Cache proof to file.
    proof_to_file(proof.clone(), "merger_wrapper", None)?;
