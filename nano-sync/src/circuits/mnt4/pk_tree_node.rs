...
impl<SubCircuit: ConstraintSynthesizer<Fr>> ConstraintSynthesizer<MNT4Fr> for PKTreeNodeCircuit<SubCircuit> {
    ...
    fn generate_constraints<CS: ConstraintSystem<MNT4Fr>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        ...
        let prepare_agg_pk_bits = SerializeGadget::serialize_g2(cs.ns(|| "serialize prepare agg pk"), &prepare_agg_pk)?;

        let pedersen_hash = PedersenHashGadget::evaluate(cs.ns(|| "prepare agg pk pedersen hash"), &prepare_agg_pk_bits, &pedersen_generators_var)?;
        ...
        let commit_agg_pk_bits = SerializeGadget::serialize_g2(cs.ns(|| "serialize commit agg pk"), &commit_agg_pk)?;

        let pedersen_hash = PedersenHashGadget::evaluate(cs.ns(|| "commit agg pk pedersen hash"), &commit_agg_pk_bits, &pedersen_generators_var)?;
        ...
        proof_inputs.append(&mut RecursiveInputGadget::to_field_elements::<Fr>(&prepare_agg_pk_chunks_commitments[2])?);

        proof_inputs.append(&mut RecursiveInputGadget::to_field_elements::<Fr>(&prepare_agg_pk_chunks_commitments[3])?);

        proof_inputs.append(&mut RecursiveInputGadget::to_field_elements::<Fr>(&commit_signer_bitmap_var)?);

        proof_inputs.append(&mut RecursiveInputGadget::to_field_elements::<Fr>(&commit_agg_pk_chunks_commitments[2])?);

        proof_inputs.append(&mut RecursiveInputGadget::to_field_elements::<Fr>(&commit_agg_pk_chunks_commitments[3])?);
