...
impl ConstraintSynthesizer<MNT4Fr> for PKTreeLeafCircuit {
    ...
    fn generate_constraints<CS: ConstraintSystem<MNT4Fr>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        ...
        let pedersen_generators_var = Vec::<G1Gadget>::alloc_constant(cs.ns(|| "alloc pedersen_generators"), pedersen_generators(195))?;
        ...
        let pks_nodes_var = Vec::<G1Gadget>::alloc(cs.ns(|| "alloc pks Merkle proof nodes"), || Ok(&self.pks_nodes[..]))?;

        let prepare_agg_pk_var = G2Gadget::alloc(cs.ns(|| "alloc prepare aggregate public key"), || Ok(&self.prepare_agg_pk))?;

        let commit_agg_pk_var = G2Gadget::alloc(cs.ns(|| "alloc commit aggregate public key"), || Ok(&self.commit_agg_pk))?;
        ...
        let prepare_agg_pk_commitment_var = UInt8::alloc_input_vec(cs.ns(|| "alloc prepare aggregate pk commitment"), self.prepare_agg_pk_commitment.as_ref())?;

        let commit_signer_bitmap_var = UInt8::alloc_input_vec(cs.ns(|| "alloc commit signer bitmap"), self.commit_signer_bitmap.as_ref())?;

        let commit_agg_pk_commitment_var = UInt8::alloc_input_vec(cs.ns(|| "alloc commit aggregate pk commitment"), self.commit_agg_pk_commitment.as_ref())?;
        ...
        let prepare_signer_bitmap_var = prepare_signer_bitmap_var.to_bits(cs.ns(|| "prepare signer bitmap to bits"))?;

        let prepare_signer_bitmap_var = prepare_signer_bitmap_var[chunk_start..chunk_end].to_vec();

        let commit_signer_bitmap_var = commit_signer_bitmap_var.to_bits(cs.ns(|| "commit signer bitmap to bits"))?;
        ...
        for i in 0..self.pks.len() {
            bits.extend(SerializeGadget::serialize_g2(cs.ns(|| format!("serialize pks: {}", i)), &pks_var[i])?);
        }
        ...
        let prepare_agg_pk_bits = SerializeGadget::serialize_g2(cs.ns(|| "serialize prepare agg pk"), &prepare_agg_pk_var)?;

        let pedersen_hash = PedersenHashGadget::evaluate(cs.ns(|| "reference prepare agg pk commitment"), &prepare_agg_pk_bits, &pedersen_generators_var)?;

        let pedersen_bits = SerializeGadget::serialize_g1(cs.ns(|| "serialize prepare pedersen hash"), &pedersen_hash)?;
        ...
        prepare_agg_pk_commitment_var.enforce_equal(cs.ns(|| "prepare agg pk commitment == reference commitment"), &reference_commitment)?;
        ...
        let commit_agg_pk_bits = SerializeGadget::serialize_g2(cs.ns(|| "serialize commit agg pk"), &commit_agg_pk_var)?;

        let pedersen_hash = PedersenHashGadget::evaluate(cs.ns(|| "reference commit agg pk commitment"), &commit_agg_pk_bits, &pedersen_generators_var)?;

        let pedersen_bits = SerializeGadget::serialize_g1(cs.ns(|| "serialize commit pedersen hash"), &pedersen_hash)?;
        ...
        commit_agg_pk_commitment_var.enforce_equal(cs.ns(|| "commit agg pk commitment == reference commitment"), &reference_commitment)?;
        ...
        for (i, (key, included)) in pks_var.iter().zip(prepare_signer_bitmap_var.iter()).enumerate() {
            // Calculate a new sum that includes the next public key.
            let new_sum = reference_prep_agg_pk.add(cs.ns(|| format!("add public key, prepare {}", i)), key)?;
        ...
        for (i, (key, included)) in pks_var.iter().zip(commit_signer_bitmap_var.iter()).enumerate() {
            // Calculate a new sum that includes the next public key.
            let new_sum = reference_comm_agg_pk.add(cs.ns(|| format!("add public key, commit {}", i)), key)?;
        ...
        prepare_agg_pk_var.enforce_equal(cs.ns(|| "verify equality prepare agg pk"), &reference_prep_agg_pk)?;

        commit_agg_pk_var.enforce_equal(cs.ns(|| "verify equality commit agg pk"), &reference_comm_agg_pk)?;
