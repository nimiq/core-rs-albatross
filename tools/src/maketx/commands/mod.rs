...
pub fn hex_option_secret_key_to_option_pair(secret_key: Option<String>) -> Result<Option<KeyPair>, CommandError> {
    Ok(match secret_key {
...
pub fn hex_to_signature_proof(signature_proof: String) -> Result<SignatureProof, CommandError> {
    Ok(SignatureProof::deserialize_from_vec(&hex::decode(signature_proof)?)?)
}

pub fn hex_secret_key_to_public(secret_key:String) -> Result<Ed25519PublicKey, CommandError> {
    Ok(Ed25519PublicKey::from(&PrivateKey::deserialize_from_vec(&hex::decode(secret_key)?)?))
}

pub fn hex_secret_key_to_bls_pair(secret_key: String) -> Result<BlsKeyPair, CommandError> {
    Ok(BlsKeyPair::from(BlsSecretKey::deserialize_from_vec(&hex::decode(secret_key)?)?))
}

pub fn hex_to_signal_data(signal_data: Option<String>) -> Result<Option<Blake2bHash>, CommandError> {
    Ok(match signal_data {
        Some(signal_data) => Some(Blake2bHash::deserialize_from_vec(&hex::decode(signal_data)?)?),
        None => None,
    })
}
...
pub enum CommandError {
    ...
    // #[error("SignatureProof")]
    // SignatuereProofError(#[from] SignatureProfError),
}
