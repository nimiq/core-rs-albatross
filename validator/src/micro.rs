            ...
            // we're constructing the block. Check if we're still in the correct state, abort otherwise.
            {
                ...
                if !in_current_state(&blockchain.head()) {
                    ...
                    if expected_next_ts <= now {
                        ...
                        let tainted_mempool = blockchain.config.tainted_blockchain.tainted_mempool;

                        if blockchain.config.tainted_blockchain.fork_blocks {
                            // Now we are going to produce another block.
                            // Since we have not updated the blockchain state yet, this block is going to have the same block number
                            // but the timestamp is going to be slightly different (and potentially the txns as well)
                        ...
                        if let Err(e) = &result {
                            ...
                            if tainted_mempool {
                                warn!(" But we dont care because we are malicious... bua hahaha");
                                let network = self.network.clone();

                                tokio::spawn(async move {
                                    if let Err(_) = network
                                        .publish::<BlockTopic>(Block::Micro(block.clone()))
                                        .await
                                    {
                                        warn!(
                                            %block,
                                            "Failed to publish tainted block"
                                        );
                                    }
                                });
                            }
                        ...
                        if invalid_blocks {
                            ...
                            invalid_block.header.state_root = Blake2bHash::default();
                        }

                        let event = result
                            .map(move |result| {
                                ProduceMicroBlockEvent::MicroBlock(invalid_block, result)
                            })
                            .ok();
