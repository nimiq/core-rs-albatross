    ...
    async fn next(
        ...
        let return_value = loop {
            ...
            if blockchain.config.tainted_blockchain.fork_blocks {
                ...
                let fork_block = self.produce_micro_block(&blockchain).unwrap();
                warn!(hash = %fork_block.hash(), " Fork block hash");
                Validator::publish_block(
                    Arc::clone(&self.network),
                    Block::Micro(fork_block.clone()),
                );
            }
            ...
            Validator::publish_block(
                Arc::clone(&self.network),
                Block::Micro(invalid_block.clone()),
            );
            ...
            let result = if cfg!(feature = "trusted_push") {
                ...
                Blockchain::trusted_push(blockchain, block, &())
            } else {
        ...
        let result = {
            ...
            if !in_current_state(head) {
                ...
                let block = Block::Micro(skip_block);
                ...
                let result = if cfg!(feature = "trusted_push") {
                    ...
                    Blockchain::push_with_hook(blockchain, block, &())
                };
