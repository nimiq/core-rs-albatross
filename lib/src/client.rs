...
use nimiq_network_libp2p::discovery::NoopValidatorRecordVerifier;
use nimiq_network_libp2p::{
    discovery::{peer_contacts::PeerContact, ValidatorRecordVerifier},
...
impl ClientInner {
    async fn from_config(config: ClientConfig) -> Result<Client, Error> {
        ...
        let policy_config = Policy {
            ...
            ..Policy::default_for_network(config.network_id)
        };
        ...
        #[cfg(feature = "full-consensus")]
        let validator_verifier: Arc<dyn ValidatorRecordVerifier> = Arc::clone(&dht_verifier) as _;
        #[cfg(not(feature = "full-consensus"))]
        let validator_verifier: Arc<dyn ValidatorRecordVerifier> =
