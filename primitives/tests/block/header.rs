...
fn it_can_calculate_genesis_block_hashes() {
    ...
    assert_eq!(header.hash::<Blake2bHash>(), Blake2bHash::from("264aaf8a4f9828a76c550635da078eb466306a189fcc03710bee9f649c869d12"));
    assert_eq!(header.hash::<Argon2dHash>(), Argon2dHash::from("000087dccfcb8625a84c821887c5c515f92f5078501bf49369f8993657cdc034"));
}

#[test]
fn it_can_calculate_block_108273_hash() {
    let header = BlockHeader::deserialize_from_vec(&hex::decode(B108273_HEADER).unwrap()).unwrap();
    assert_eq!(header.hash::<Blake2bHash>(), Blake2bHash::from("65631e10f76ac8e95ec0766e84ec2be46818e2351b0174220aab7fc7243fca17"));
    assert_eq!(header.hash::<Argon2dHash>(), Argon2dHash::from("00000000082480b610551e47995125087532c3ea3865d2846e53cf32184f01e4"));
}

#[test]
fn it_can_calculate_block_169500_hash() {
    let header = BlockHeader::deserialize_from_vec(&hex::decode(B169500_HEADER).unwrap()).unwrap();
    assert_eq!(header.hash::<Blake2bHash>(), Blake2bHash::from("3c084e90460d0313e87d9dfe3d4bafd74cab083d426529cc17e94df3be548f59"));
    assert_eq!(header.hash::<Argon2dHash>(), Argon2dHash::from("000000000c0d13dca4b49b3ecfb9a734fad2fc9ffea7d49ee8a6832fc4c6200e"));
}
...
fn verify_rejects_invalid_proof_of_work() {
    let mut header1 = BlockHeader::deserialize_from_vec(&hex::decode(GENESIS_HEADER).unwrap()).unwrap();
    header1.nonce = 1;
    assert!(!header1.verify_proof_of_work());

    let mut header2: BlockHeader = BlockHeader::deserialize_from_vec(&hex::decode(B108273_HEADER).unwrap()).unwrap();
