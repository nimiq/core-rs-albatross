...
fn jump_skip_that_overflows_the_program_counter_terminates() {
    // `pc` is 1 at the jump, so `1 + (usize::MAX - 1)` lands exactly on
    // `usize::MAX` and the loop's own increment wraps it back to 0. The stack
    // stays balanced across passes, so nothing ever errors out to break the
    // cycle: unfixed, this spins forever in release and panics in debug.
...
fn maximal_jump_skip_terminates() {
    // The largest possible skip. It cannot spin forever on its own — the wrap
    // lands back on the jump itself, which drains one stack value per pass — but
    // it does overflow the addition, which panics under debug assertions.
    assert_terminates(ValidationProgram::new(vec![
        ValidationOp::PushConst(0),
