...
/// A bridge is bound to one source chain, and two fields name it: `source_chain_id` on the
/// creation data, which the release path compares every burn's declared chain against, and
/// `chain_id` inside the `ChainConfig`, which nothing reads once the bridge exists. Creation
/// verification requires the two to agree and refuses a zero id; this test pins both checks.
///
/// A zero `source_chain_id` would lock funds outright: `ParsedBurnData` refuses a burn declaring
/// chain 0, so no release could ever match and the bridge would take deposits it can never pay
/// out. A disagreeing `chain_id` would be quieter: since nothing reads it after creation, a config
/// describing one chain while the bridge enforces another can only be caught here.
#[test]
