...
use nimiq_keys::{Address, ES256PublicKey, ES256Signature, PublicKey, SignatureEnum};
...
fn it_can_serialize_and_deserialize_signature_proofs() {
    let webauthn_sp = SignatureProof {
    ...
    let serialized = Serialize::serialize_to_vec(&webauthn_sp.clone());

    let proof: SignatureProof = Deserialize::deserialize_from_vec(&serialized[..]).unwrap();
    ...
    assert_eq!(proof.webauthn_fields, webauthn_sp.webauthn_fields);
}
