...
impl StakingContract {
    ...
    pub(super) fn revert_retire_recipient(&mut self, staker_address: &Address, value: Coin, receipt: Option<InactiveStakeReceipt>) -> Result<(), AccountError> {
        let inactive_stake = self.inactive_stake_by_address.get(staker_address).ok_or(AccountError::InvalidForRecipient)?;
    ...
    pub(super) fn reactivate_sender(
        ...
        let inactive_stake = self.inactive_stake_by_address.remove(staker_address).ok_or(AccountError::InvalidForRecipient)?;
    ...
    pub(super) fn unstake(&mut self, staker_address: &Address, total_value: Coin) -> Result<Option<InactiveStakeReceipt>, AccountError> {
        let inactive_stake = self.inactive_stake_by_address.remove(staker_address).ok_or(AccountError::InvalidForSender)?;
