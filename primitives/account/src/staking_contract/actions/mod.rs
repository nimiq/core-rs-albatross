        ...
        if transaction.sender != transaction.recipient {
            ...
            let receipt: Option<Vec<u8>> = match data {
                ...
                IncomingStakingTransactionData::UpdateValidator {
                    old_validator_key, new_validator_key, new_reward_address, ..
                } => {
            ...
            match data {
                SelfStakingTransactionData::RetireStake(_) => {
                    // Retire transaction.
                    Ok(self.retire_recipient(&staker_address, transaction.value, Some(block_height))?
        ...
        if transaction.sender != transaction.recipient {
            ...
            match proof {
                ...
                OutgoingStakingTransactionProof::DropValidator { validator_key, .. } => {
                    let inactive_validator = self.inactive_validators_by_key.get(&validator_key)
                        .ok_or(AccountError::InvalidForSender)?;

                    // Check unstake delay.
                    if block_height < policy::macro_block_after(inactive_validator.retire_time) + policy::UNSTAKING_DELAY {
                        return Err(AccountError::InvalidForSender);
                    }

                    // Check stakes.
                    let validator = &inactive_validator.validator;
                    let staker_stake = validator.active_stake_by_address.read().values()
                        .cloned().fold(Coin::ZERO, Add::add);
            ...
            match data {
                SelfStakingTransactionData::RetireStake(validator_key) => {
                    ...
                    let stake = stakes.get(&staker_address)
                        .ok_or(AccountError::InvalidForSender)?;
    ...
    fn commit_outgoing_transaction(&mut self, transaction: &Transaction, block_height: u32) -> Result<Option<Vec<u8>>, AccountError> {
        ...
        if transaction.sender != transaction.recipient {
            ...
            Ok(match proof {
                ...
                OutgoingStakingTransactionProof::DropValidator { validator_key, .. } => {
                    Some(self.drop_validator(&validator_key, transaction.total_value()?)?
                        .serialize_to_vec())
                },
            })
        } else {
            ...
            Ok(match data {
                ...
                SelfStakingTransactionData::ReactivateStake(_validator_key) => {
                    self.reactivate_sender(&staker_address, transaction.total_value()?, None)?
                        .map(|r| r.serialize_to_vec())
                },
            })
        }
    ...
    fn check_inherent(&self, inherent: &Inherent, _block_height: u32) -> Result<(), AccountError> {
        ...
        match inherent.ty {
            InherentType::Slash => {
                ...
                if !self.active_validators_by_key.contains_key(&validator_key) && !self.inactive_validators_by_key.contains_key(&validator_key) {
                    return Err(AccountError::InvalidInherent);
                }
