...
use nimiq_bls::bls12_381::SecretKey as BlsSecretKey;
...
const VALIDATOR_SECRET_KEY: &str = "49ea68eb6b8afdf4ca4d4c0a0b295c76ca85225293693bc30e755476492b707f";
...
fn it_can_de_serialize_a_staking_contract() {
    ...
    let key = BlsPublicKey::deserialize_from_vec(
        &hex::decode(VALIDATOR_KEY).unwrap()
    ).unwrap();
    ...
    assert_eq!(validator.balance,
               Coin::from_u64_unchecked(600_000_000));
    assert_eq!(validator.active_stake_by_address.read().len(), 2);
    assert_eq!(contract_2.get_active_stake(&key, &Address::from([2u8; 20])),
               Some(Coin::from_u64_unchecked(200_000_000u64)));
    assert_eq!(contract_2.get_active_stake(&key, &Address::from([0x5eu8; 20])),
...
fn it_can_verify_validator_and_staking_transaction() {
    let validator_key = BlsPublicKey::deserialize_from_vec(
        &hex::decode(VALIDATOR_KEY).unwrap()
    ).unwrap();
    let keypair = bls_key_pair();

    let mut tx = make_incoming_transaction(IncomingStakingTransactionData::CreateValidator {
    ...
    let tx = make_incoming_transaction(IncomingStakingTransactionData::CreateValidator {
        validator_key: validator_key.clone(),
    ...
    // Staking
    let mut tx = make_incoming_transaction(IncomingStakingTransactionData::Stake {
        validator_key: validator_key.clone(),
        staker_address: None,
    }, 100_000_000);
...
fn it_can_apply_validator_and_staking_transaction() {
    ...
    // Create validator
    let tx_1 = make_incoming_transaction(IncomingStakingTransactionData::CreateValidator {
    ...
    assert_eq!(contract.get_validator(&validator_key).unwrap().balance,
               Coin::from_u64_unchecked(150_000_000));
    assert_eq!(contract.balance, 150_000_000.try_into().unwrap());

    // Cannot overwrite validator
    assert_eq!(contract.commit_incoming_transaction(&tx_1, 2), Err(AccountError::InvalidForRecipient));

    // Stake
    let tx_2 = make_incoming_transaction(IncomingStakingTransactionData::Stake {
        validator_key: validator_key.clone(),
        staker_address: None,
    }, 150_000_000);
    ...
    assert_eq!(contract.get_active_stake(&validator_key, &staker_address).unwrap(),
               Coin::from_u64_unchecked(150_000_000));
    assert_eq!(contract.balance, 300_000_000.try_into().unwrap());

    // Stake again, for different address
    let tx_3 = make_incoming_transaction(IncomingStakingTransactionData::Stake {
        validator_key: validator_key.clone(),
    ...
    assert_eq!(contract.get_active_stake(&validator_key, &staker_address).unwrap(),
               Coin::from_u64_unchecked(150_000_000));
    assert_eq!(contract.get_active_stake(&validator_key, &Address::from([2u8; 20])).unwrap(),
               Coin::from_u64_unchecked(150_000_000));
    assert_eq!(contract.balance, 450_000_000.try_into().unwrap());

    // Stake more
    let tx_4 = make_incoming_transaction(IncomingStakingTransactionData::Stake {
        validator_key: validator_key.clone(),
        staker_address: None,
    }, 150_000_000);
...
fn test_proof_verification(transaction: Transaction) {
    ...
    let other_pair = KeyPair::from(PrivateKey::deserialize_from_vec(
        &hex::decode("5d205b80adbffc32fe21927c7fd119623d44746a3ddd35a299a1e483c3402cd9").unwrap()
    ).unwrap());

    if transaction.recipient_type != AccountType::Staking {
        // More complex proof.
        let data: OutgoingStakingTransactionProof = Deserialize::deserialize_from_vec(&transaction.proof[..]).unwrap();
        match data {
            OutgoingStakingTransactionProof::Unstake(_) => {
                tx_3.proof = OutgoingStakingTransactionProof::Unstake(
            ...
            OutgoingStakingTransactionProof::DropValidator { validator_key, .. } => {
                let bls_pair = BlsKeyPair::from(
                    BlsSecretKey::deserialize_from_vec(
                        &hex::decode("59d1283e749367003fd63bb92e5f6914a5845da093adfb91779c2fbc43f8ec6e").unwrap()
                    ).unwrap());
    ...
    // Valid
    assert_eq!(AccountType::verify_outgoing_transaction(&transaction), Ok(()));
    if transaction.recipient_type == AccountType::Staking {
...
fn it_can_apply_retiring_transaction() {
    ...
    let tx_1 = make_self_transaction(
        SelfStakingTransactionData::RetireStake(validator_key.clone()), 99_999_900
    );
    ...
    assert_eq!(contract.get_active_stake(&validator_key, &staker_address), Some(Coin::from_u64_unchecked(50_000_000)));
    assert_eq!(contract.inactive_stake_by_address.len(), 1);
    assert_eq!(contract.inactive_stake_by_address.get(&staker_address).unwrap().balance, Coin::from_u64_unchecked(99_999_900));
    ...
    let tx_3 = make_self_transaction(
        SelfStakingTransactionData::RetireStake(validator_key.clone()), 24_999_900
    );
    ...
    assert_eq!(contract.inactive_stake_by_address.len(), 1);
    assert_eq!(contract.inactive_stake_by_address.get(&staker_address).unwrap().balance, Coin::from_u64_unchecked(149_999_700));
    ...
    assert_eq!(contract.get_active_stake(&validator_key, &staker_address), Some(Coin::from_u64_unchecked(150_000_000)));
...
fn it_can_verify_unstaking_and_drop_transactions() {
    ...
    test_proof_verification(make_unstake_transaction(
        &key_pair,
        10
    ));
    test_proof_verification(make_drop_transaction(
        &bls_pair,
        10
    ));
}
...
fn it_can_apply_unstaking_transaction() {
    ...
    let tx_1 = make_self_transaction(SelfStakingTransactionData::RetireStake(bls_pair.public.compress()), 50_000_000);
    assert_eq!(contract.commit_outgoing_transaction(&tx_1, 2), Ok(None));
    assert_eq!(contract.commit_incoming_transaction(&tx_1, 2), Ok(None));
    assert_eq!(contract.balance, Coin::from_u64_unchecked(299_999_900));
    assert_eq!(contract.get_active_stake(&bls_pair.public.compress(), &recipient), Some(Coin::from_u64_unchecked(99_999_900)));
    assert_eq!(contract.inactive_stake_by_address.get(&recipient).unwrap().balance, Coin::from_u64_unchecked(50_000_000));
    ...
    assert_eq!(contract.get_active_stake(&bls_pair.public.compress(), &recipient), Some(Coin::from_u64_unchecked(99_999_900)));
    assert_eq!(contract.inactive_stake_by_address.get(&recipient).unwrap().balance, Coin::from_u64_unchecked(37_500_000));
    ...
    assert_eq!(contract.get_active_stake(&bls_pair.public.compress(), &recipient), Some(Coin::from_u64_unchecked(99_999_900)));
    assert_eq!(contract.inactive_stake_by_address.get(&recipient).unwrap().balance, Coin::from_u64_unchecked(25_000_000));

    // Revert block 40004
    assert_eq!(contract.revert_outgoing_transaction(&tx_4, 40004, None), Ok(()));
    assert_eq!(contract.balance, Coin::from_u64_unchecked(287_499_900));
    assert_eq!(contract.get_active_stake(&bls_pair.public.compress(), &recipient), Some(Coin::from_u64_unchecked(99_999_900)));
    assert_eq!(contract.inactive_stake_by_address.get(&recipient).unwrap().balance, Coin::from_u64_unchecked(37_500_000));

    // New block 40004: Retire second half of stake
    let tx_5 = make_self_transaction(SelfStakingTransactionData::RetireStake(bls_pair.public.compress()), 99_999_800);
    ...
    assert_eq!(contract.get_active_stake(&bls_pair.public.compress(), &recipient), None);
    assert_eq!(contract.inactive_stake_by_address.get(&recipient).unwrap().balance, Coin::from_u64_unchecked(137_499_800));
    ...
    assert_eq!(contract.revert_outgoing_transaction(&tx_5, 40004, None), Ok(()));
    ...
    assert_eq!(contract.inactive_stake_by_address.len(), 0);
    assert_eq!(contract.balance, Coin::from_u64_unchecked(300_000_000));
    assert_eq!(contract.get_active_stake(&bls_pair.public.compress(), &recipient), Some(Coin::from_u64_unchecked(150_000_000)));
}
...
fn it_can_apply_unpark_transactions() {
    ...
    let unpark = make_signed_incoming_transaction(IncomingStakingTransactionData::UnparkValidator {
        validator_key: validator_key.clone(),
        signature: Default::default()
    }, 100, &bls_pair);
    ...
    unpark.data = Vec::new();
    if let Err(TransactionError::InvalidSerialization(_e)) = AccountType::verify_incoming_transaction(&unpark) {
        // Ok
    } else {
...
fn it_can_build_a_validator_set() {
    ...
    let bls_key2 = BlsKeyPair::from(
        BlsSecretKey::deserialize_from_vec(
            &hex::decode("12434643b255b86c780670ede72500a84de5ce633f674c799e27b09187d5a414").unwrap()
        ).unwrap());
    let validator2 = bls_key2.public.compress();
    let bls_key3 = BlsKeyPair::from(
        BlsSecretKey::deserialize_from_vec(
            &hex::decode("3f651a48337d8e24edca32b3605e91ce0d43e7311dcdc61ec000b533a6bc907e").unwrap()
        ).unwrap());
...
fn it_can_apply_validator_signalling() {
    ...
    // Check revert.
    let mut contract_copy = contract.clone();
    assert_eq!(contract_copy.revert_incoming_transaction(&tx, 2, None), Ok(()));
    ...
    contract.stake(Address::from(&key_pair),
                   Coin::from_u64_unchecked(100_000_000), &validator_key).unwrap();

    // 1a. Update the validator.
    let mut contract_copy = contract.clone();

    // Incorrect proof of knowledge.
    let tx = make_signed_incoming_transaction(IncomingStakingTransactionData::UpdateValidator {
    ...
    let receipt = contract_copy
        .commit_incoming_transaction(&tx, 3)
        .unwrap();
    ...
    // Revert update.
    assert_eq!(contract_copy.revert_incoming_transaction(&tx, 3, receipt.as_ref()), Ok(()));
    ...
    let tx = make_signed_incoming_transaction(IncomingStakingTransactionData::RetireValidator {
        validator_key: validator_key.clone(),
        signature: Default::default(),
    }, 0, &bls_pair);
    assert_eq!(AccountType::verify_incoming_transaction(&tx), Ok(()));
    ...
    assert_eq!(&validator.validator_key, &validator_key);
    assert_eq!(validator.reward_address, Address::from([3u8; 20]));
    let inactive_validator = contract.inactive_validators_by_key
        .get(&validator_key)
        .unwrap();
    ...
    let mut contract_copy = contract.clone();
    let tx = make_signed_incoming_transaction(IncomingStakingTransactionData::ReactivateValidator {
        validator_key: validator_key.clone(),
        signature: Default::default(),
    }, 0, &bls_pair);
    assert_eq!(AccountType::verify_incoming_transaction(&tx), Ok(()));
    ...
    let receipt = contract_copy
        .commit_incoming_transaction(&tx, 4)
        .unwrap();
    assert!(receipt.is_some());
    ...
    assert_eq!(contract_copy.revert_incoming_transaction(&tx, 2, receipt.as_ref()), Ok(()));
    ...
    assert_eq!(&validator.validator_key, &validator_key);
    assert_eq!(validator.reward_address, Address::from([3u8; 20]));
    let inactive_validator = contract_copy.inactive_validators_by_key
        .get(&validator_key)
        .unwrap();
    assert_eq!(inactive_validator.retire_time, 3);
    ...
    assert_eq!(reactivated_contract.check_outgoing_transaction(&tx, 3000), Err(AccountError::InvalidForSender));
    assert_eq!(reactivated_contract.commit_outgoing_transaction(&tx, 3000), Err(AccountError::InvalidForSender));
    ...
    assert_eq!(AccountType::verify_outgoing_transaction(&tx), Ok(()));
    assert_eq!(contract.check_outgoing_transaction(&tx, 3000), Err(AccountError::InsufficientFunds {
        needed: Coin::from_u64_unchecked(300_000_100),
        balance: Coin::from_u64_unchecked(200_000_000),
    }));
    assert_eq!(contract.commit_outgoing_transaction(&tx, 3000), Err(AccountError::InsufficientFunds {
        needed: Coin::from_u64_unchecked(300_000_100),
        balance: Coin::from_u64_unchecked(200_000_000),
    }));
    ...
    let tx = make_drop_transaction(&bls_pair, 99_999_900);
    assert_eq!(AccountType::verify_outgoing_transaction(&tx), Ok(()));
    assert_eq!(contract.check_outgoing_transaction(&tx, 4), Err(AccountError::InvalidForSender));
    assert_eq!(contract.commit_outgoing_transaction(&tx, 4), Err(AccountError::InvalidForSender));

    // All valid.
    let tx = make_drop_transaction(&bls_pair, 99_999_900);
    assert_eq!(AccountType::verify_outgoing_transaction(&tx), Ok(()));
    assert_eq!(contract.check_outgoing_transaction(&tx, 3000), Ok(()));
    let receipt = contract.commit_outgoing_transaction(&tx, 3000)
        .unwrap();
    assert!(receipt.is_some());
    ...
    assert_eq!(contract.inactive_validators_by_key.len(), 0);
    assert_eq!(contract.inactive_stake_by_address.len(), 1);
    let inactive_stake = contract.inactive_stake_by_address.get(&Address::from(&key_pair))
        .unwrap();
    ...
    // Check revert.
    let mut contract_copy = contract.clone();
    assert_eq!(contract_copy.revert_outgoing_transaction(&tx, 2, receipt.as_ref()), Ok(()));
...
fn it_can_manage_stake() {
    ...
    // --- Active validator ---
    // 1. Stake.
    let tx = make_incoming_transaction(IncomingStakingTransactionData::Stake {
        validator_key: validator_key.clone(),
        staker_address: None,
    }, 100_000_000);
    assert_eq!(AccountType::verify_incoming_transaction(&tx), Ok(()));
    ...
    assert_eq!(validator.balance, Coin::from_u64_unchecked(200_000_000));
    assert_eq!(validator.active_stake_by_address.read().len(), 1);
    assert_eq!(validator.active_stake_by_address.read().get(&staker_address), Some(&Coin::from_u64_unchecked(100_000_000)));
    ...
    assert_eq!(validator.active_stake_by_address.read().len(), 1);
    assert_eq!(validator.active_stake_by_address.read().get(&staker_address), Some(&Coin::from_u64_unchecked(200_000_000)));

    // Test stake for other address.
    let mut contract_copy = contract.clone();
    let tx2 = make_incoming_transaction(IncomingStakingTransactionData::Stake {
        validator_key: validator_key.clone(),
    ...
    assert_eq!(validator.active_stake_by_address.read().len(), 2);
    assert_eq!(validator.active_stake_by_address.read().get(&staker_address), Some(&Coin::from_u64_unchecked(200_000_000)));
    assert_eq!(validator.active_stake_by_address.read().get(&Address::from([3u8; 20])), Some(&Coin::from_u64_unchecked(100_000_000)));
    ...
    // Revert stakes.
    let mut contract_copy = contract.clone();
    assert_eq!(contract_copy.revert_incoming_transaction(&tx, 1, None), Ok(()));
    ...
    let tx = make_self_transaction(
        SelfStakingTransactionData::RetireStake(validator_key.clone()), 49_999_900
    );
    assert_eq!(AccountType::verify_outgoing_transaction(&tx), Ok(()));
    ...
    assert_eq!(validator.active_stake_by_address.read().len(), 1);
    assert_eq!(validator.active_stake_by_address.read().get(&staker_address), Some(&Coin::from_u64_unchecked(50_000_000)));
    let inactive_stake = contract.inactive_stake_by_address.get(&staker_address).unwrap();
    ...
    // Revert retire.
    assert_eq!(contract_copy.revert_incoming_transaction(&tx, 3, receipt.as_ref()), Ok(()));
    assert_eq!(contract_copy.revert_outgoing_transaction(&tx, 3, None), Ok(()));
    ...
    assert_eq!(contract_copy.revert_incoming_transaction(&tx, 2, None), Ok(()));
    assert_eq!(contract_copy.revert_outgoing_transaction(&tx, 2, None), Ok(()));
    ...
    let tx = make_self_transaction(
        SelfStakingTransactionData::ReactivateStake(validator_key2.clone()), 29_999_800
    );
    assert_eq!(AccountType::verify_outgoing_transaction(&tx), Ok(()));
    ...
    assert_eq!(validator1.active_stake_by_address.read().get(&staker_address), Some(&Coin::from_u64_unchecked(50_000_000)));
    ...
    assert_eq!(validator2.active_stake_by_address.read().get(&staker_address), Some(&Coin::from_u64_unchecked(29_999_800)));
    let inactive_stake = contract.inactive_stake_by_address.get(&staker_address).unwrap();
    ...
    let tx2 = make_self_transaction(
        SelfStakingTransactionData::ReactivateStake(validator_key2.clone()), 19_999_900
    );
    ...
    assert_eq!(validator2.active_stake_by_address.read().len(), 1);
    assert_eq!(validator2.active_stake_by_address.read().get(&staker_address), Some(&Coin::from_u64_unchecked(49_999_700)));

    // Revert re-activation one.
    assert_eq!(contract_copy.revert_incoming_transaction(&tx2, 4, None), Ok(()));
    assert_eq!(contract_copy.revert_outgoing_transaction(&tx2, 4, receipt.as_ref()), Ok(()));
    ...
    assert_eq!(contract_copy.revert_incoming_transaction(&tx, 3, None), Ok(()));
    assert_eq!(contract_copy.revert_outgoing_transaction(&tx, 3, None), Ok(()));
    ...
    assert_eq!(AccountType::verify_outgoing_transaction(&tx), Ok(()));
    assert_eq!(contract.check_outgoing_transaction(&tx, 3), Err(AccountError::InvalidForSender));
    assert_eq!(contract.commit_outgoing_transaction(&tx, 3), Err(AccountError::InvalidForSender));
    ...
    let receipt = contract_copy.commit_outgoing_transaction(&tx2, 3000).unwrap();
    assert!(receipt.is_some());
    ...
    // Revert unstake.
    assert_eq!(contract_copy.revert_outgoing_transaction(&tx2, 3000, receipt.as_ref()), Ok(()));
    ...
    assert_eq!(contract_copy.revert_outgoing_transaction(&tx, 3000, None), Ok(()));
...
fn make_unstake_transaction(key_pair: &KeyPair, value: u64) -> Transaction {
    ...
    let proof = OutgoingStakingTransactionProof::Unstake(
        SignatureProof::from(key_pair.public.clone(),
...
fn bls_key_pair() -> BlsKeyPair {
    BlsKeyPair::from(
        BlsSecretKey::deserialize_from_vec(
            &hex::decode(VALIDATOR_SECRET_KEY).unwrap()
        ).unwrap())
}

fn ed25519_key_pair() -> KeyPair {
    KeyPair::from(PrivateKey::deserialize_from_vec(
        &hex::decode(STAKER_PRIVATE_KEY).unwrap()
    ).unwrap())
}
