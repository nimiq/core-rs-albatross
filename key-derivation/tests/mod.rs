...
impl TestVector {
    ...
    fn get_public_key(&self) -> PublicKey {
        let mut public_key: [u8; PublicKey::SIZE] = Default::default();
...
fn it_correctly_derives_keys() {
    ...
    for (i, vectors) in TEST_CASES.iter().enumerate() {
        ...
        for (j, vector) in vectors.iter().enumerate() {
            ...
            assert_eq!(key.get_chain_code(), &vector.get_chain_code(), "Invalid chain code for seed {} in test case {}", i, j);

            assert_eq!(&key.to_public_key(), &vector.get_public_key(), "Invalid public key for seed {} in test case {}", i, j);

            assert_eq!(&key.into_private_key(), &vector.get_private_key(), "Invalid private key for seed {} in test case {}", i, j);
        }
    }
}
