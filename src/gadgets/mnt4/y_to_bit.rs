...
use algebra::{
    mnt4_753::Fr as MNT4Fr,
    mnt6_753::{Fq, MNT6_753},
    One, PrimeField,
};
...
pub struct MNT4YToBitGadget;

impl YToBitGadget<MNT6_753, MNT4Fr, PairingGadget> for MNT4YToBitGadget {
    ...
    fn y_to_bit_g1<CS: r1cs_core::ConstraintSystem<MNT4Fr>>(
        mut cs: CS,
        point: &G1Gadget,
    ...
    /// where half means the half point of the modulus of the underlying field. So, half = (p-1)/2.
    fn y_to_bit_g2<CS: r1cs_core::ConstraintSystem<MNT4Fr>>(
        mut cs: CS,
        point: &G2Gadget,
    ) -> Result<Boolean, SynthesisError> {
        // Calculates -half.
