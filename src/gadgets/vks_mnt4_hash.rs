use algebra::mnt4_753::{Fq as MNT4Fq, FqParameters, MNT4_753};
...
use r1cs_std::mnt4_753::G1Gadget;
use r1cs_std::prelude::mnt4_753::PairingGadget as MNT4PairingGadget;
...
pub struct VKsMNT4HashGadget;

impl VKsMNT4HashGadget {
    ...
    pub fn evaluate<CS: r1cs_core::ConstraintSystem<MNT6Fr>>(
        mut cs: CS,
        vk_macro_block: VerifyingKeyGadget<MNT4_753, MNT4Fq, MNT4PairingGadget>,
        vk_merger: VerifyingKeyGadget<MNT4_753, MNT4Fq, MNT4PairingGadget>,
    ) -> Result<Vec<UInt32>, SynthesisError> {
        // Initialize Boolean vector.
        let mut bits: Vec<Boolean> = vec![];

        // Convert vk_macro_block to bits.
        // First the alpha_g1 point.
    ...
    pub fn g1_to_bits<CS: r1cs_core::ConstraintSystem<MNT6Fr>>(
        mut cs: CS,
        bits: &mut Vec<Boolean>,
        point: &G1Gadget,
    ) -> Result<(), SynthesisError> {
        // Get bits from the x coordinate.
        let x_bits: Vec<Boolean> = point.x.to_bits(cs.ns(|| "x to bits"))?;

        // Get one bit from the y coordinate.
        let greatest_bit = YToBitGadget::y_to_bit_g1(cs.ns(|| "y to bit"), point)?;
