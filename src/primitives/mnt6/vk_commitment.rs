...
use crate::constants::sum_generator_g1_mnt4;
use crate::primitives::mnt6::{pedersen_commitment, pedersen_generators};
use crate::utils::{bytes_to_bits, serialize_g1_mnt4, serialize_g2_mnt4};
...
pub fn vk_commitment(vk: VerifyingKey<MNT4_753>) -> Vec<u8> {
    // Serialize the verifying key into bits.
    let mut bytes: Vec<u8> = vec![];
    bytes.extend_from_slice(serialize_g1_mnt4(vk.alpha_g1.into_projective()).as_ref());
    bytes.extend_from_slice(serialize_g2_mnt4(vk.beta_g2.into_projective()).as_ref());
    bytes.extend_from_slice(serialize_g2_mnt4(vk.gamma_g2.into_projective()).as_ref());
    bytes.extend_from_slice(serialize_g2_mnt4(vk.delta_g2.into_projective()).as_ref());
    for i in 0..vk.gamma_abc_g1.len() {
        bytes.extend_from_slice(serialize_g1_mnt4(vk.gamma_abc_g1[i].into_projective()).as_ref());
    }
    ...
    let sum_generator = sum_generator_g1_mnt4();
    ...
    let bytes = serialize_g1_mnt4(pedersen_commitment);
