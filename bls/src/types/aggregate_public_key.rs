...
/// An aggregate public key. Mathematically, it is equivalent to a regular public key. However, we
/// created a new type for it in order to help differentiate between the two use cases.
#[derive(Clone, Copy)]
pub struct AggregatePublicKey(pub(crate) PublicKey);

impl AggregatePublicKey {
    /// Creates a new "empty" aggregate public key. It is simply the identity element of the
    /// elliptic curve, also known as the point at infinity.
    pub fn new() -> Self {
        AggregatePublicKey(PublicKey {
            public_key: G1Projective::zero(),
        })
    }

    /// Creates an aggregated public key from an array of regular public keys.
    /// When using this method, it is essential that there exist proofs of knowledge of the secret
    /// key for each public key. Otherwise, an adversary can submit a public key to cancel out other
    /// public keys. This is called a "rogue key attack".
    pub fn from_public_keys(public_keys: &[PublicKey]) -> Self {
        let mut agg_key = G1Projective::zero();
