...
use nimiq_block_albatross::{
    Block, BlockError, ForkProof, MacroBlock, MacroBody, MacroHeader, MultiSignature, SignedViewChange, TendermintIdentifier, TendermintProof, TendermintStep,
...
fn sign_view_change(prev_seed: VrfSeed, block_number: u32, new_view_number: u32) -> ViewChangeProof {
    ...
    // Create multisignature.
    let multisig = MultiSignature {
        signature: AggregateSignature::from_signatures(&*vec![signed_view_change; TWO_THIRD_SLOTS as usize]),
        signers,
    };
...
fn it_can_produce_election_blocks() {
    ...
    while policy::epoch_at(blockchain.block_number()) < 2 {
        ...
        let macro_block = producer.next_macro_block_proposal(blockchain.time.now() + blockchain.block_number() as u64 * 1000, 0u32, vec![0x42]);
