...
use json::{array, object, Array, JsonValue, Null};
...
fn handle_request<H>(handler: Arc<H>, str_o: Result<&str, std::str::Utf8Error>) -> Response<Body>
where
    H: Handler,
{
    ...
    for msg in json.members() {
        ...
        let result_o = handler.call_method(msg["method"].as_str().unwrap(), params_array);
        if result_o.is_none() {
    ...
    if single {
        ...
        builder
            .body(Body::from(json::stringify(JsonValue::Array(results))))
            .unwrap()
    }
}

fn check_authentication<H: Handler>(
    handler: Arc<H>,
    ...
    if let Some(authorization) = authorization {
        ...
        let authorization =
            base64::decode(authorization[1]).map_err(|_| AuthenticationError::InvalidHeader)?;
        let authorization = std::str::from_utf8(authorization.as_slice())
            .map_err(|_| AuthenticationError::InvalidHeader)?
            .split(':')
            .collect::<Vec<&str>>();
...
impl<H> IntoFuture for Service<H>
where
    H: Handler,
{
    ...
    H: Handler + 'static,
{
    ...
    fn call(
        ...
        match *req.method() {
            Method::GET => Box::new(future::ok(Response::new(Body::from(
                "Nimiq JSON-RPC Server",
            )))),
            Method::POST => {
                if let Err(e) =
                    check_authentication(Arc::clone(&handler), req.headers().get("Authorization"))
                {
