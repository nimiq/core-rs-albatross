...
impl NetworkHandler {
    pub fn new(consensus: &Arc<Consensus<Network>>) -> Self {
        NetworkHandler {
            ...
            starting_block: consensus.blockchain.block_number(),
        }
    }
    ...
    pub(crate) fn syncing(&self, _params: &[JsonValue]) -> Result<JsonValue, JsonValue> {
        Ok(if self.consensus.established() {
            ...
            let current_block = self.blockchain.block_number();
            object! {
    ...
    pub(crate) fn peer_state(&self, params: &[JsonValue]) -> Result<JsonValue, JsonValue> {
        let peer_uri = params
            ...
            .and_then(|uri| {
                PeerUri::from_str(uri).map_err(|e| object! {"message" => e.to_string()})
            })?;

        let peer_id = peer_uri
            .peer_id()
            .ok_or_else(|| object! {"message" => "URI must contain peer ID"})
            .and_then(|s| PeerId::from_str(s).map_err(|e| object! {"message" => e.to_string()}))?;

        let mut address_book = self.network.addresses.state_mut();
        let peer_address = address_book
            .get_by_peer_id(&peer_id)
            .ok_or_else(|| object! {"message" => "Unknown peer"})?;
        let mut peer_address_info = address_book
            .get_info_mut(&peer_address)
            .ok_or_else(|| object! {"message" => "Unknown peer"})?;

        let connection_pool = self.network.connections.state();
        let connection_info =
            connection_pool.get_connection_by_peer_address(&peer_address_info.peer_address);
        ...
        if !set.is_null() {
            let set = set
                .as_str()
                .ok_or_else(|| object! {"message" => "Invalid value for 'set'"})?;
            match set {
                "disconnect" => {
                    if let Some(p) = peer_channel {
    ...
    pub(crate) fn peer_address_info_to_obj(
        ...
        let connection_info = connection_info
            .or_else(|| state.get_connection_by_peer_address(&peer_address_info.peer_address));
