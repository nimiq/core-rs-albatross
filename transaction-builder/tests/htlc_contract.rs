...
fn it_can_create_creation_transaction() {
    ...
    htlc_builder.with_sender(sender.clone())
...
fn prepare_outgoing_transaction() -> (Transaction, AnyHash, AnyHash, KeyPair, SignatureProof, KeyPair, SignatureProof) {
    ...
    return (tx, pre_image, hash_root, sender_key_pair, sender_signature_proof, recipient_key_pair, recipient_signature_proof)
}
...
fn it_can_create_regular_transfer() {
    ...
    let mut builder = TransactionBuilder::new();
    builder.with_sender(Address::from([0u8; 20]))
    ...
    proof_builder.regular_transfer(HashAlgorithm::Blake2b, pre_image.clone(), 1, hash_root.clone(), proof);
