...
impl<N: Network, M: MacroSync<N::PeerId>, L: LiveSync<N>> Syncer<N, M, L> {
    ...
    async fn is_peer_synced(
        ...
        let election_block = match blockchain.get_block(&head.election, false) {
            Ok(block) => block,
            Err(_) => {
                debug!(%peer_id, election_head = %head.election, "Incompatible peer's election block not found");
                return false;
            }
        };
        ...
        let synced =
            checkpoint.batch_number() >= blockchain.macro_head().batch_number().saturating_sub(1);
        if !synced {
            debug!(%peer_id, peers_batch = checkpoint.batch_number(), our_batch = blockchain.macro_head().batch_number(), "Incompatible peer is in a different batch");
        }
        synced
    }
}
