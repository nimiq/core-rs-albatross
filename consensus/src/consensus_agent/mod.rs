...
impl<P: ConsensusProtocol + 'static> ConsensusAgent<P> {
    ...
    const CHAIN_PROOF_RATE_LIMIT: usize = 3;
    // per minute
    const BLOCK_PROOF_RATE_LIMIT: usize = 60;
    // per minute
    const TRANSACTION_RECEIPTS_RATE_LIMIT: usize = 30;
    // per minute
    const TRANSACTIONS_PROOF_RATE_LIMIT: usize = 60;
    // per minute
    const ACCOUNTS_PROOF_RATE_LIMIT: usize = 60;
    // per minute
    ...
    const MEMPOOL_DELAY_MIN: u64 = 2 * 1000;
    // in ms
