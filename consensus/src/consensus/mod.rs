...
pub struct RateLimit {
    ...
    allowed_occurrences: u16,
    /// The range of time/blocks.
    block_range: u32,
    /// The block number of the last reset.
    last_reset: u32,
    /// The counter of requests submited within the current block range.
    occurrences_counter: u16,
}

impl RateLimit {
    pub fn new(allowed_occurrences: u16, _block_range: u32) -> Self {
        RateLimit {
            allowed_occurrences,
    ...
    /// Updates the block_range if needed and then increments the counter of number of requests by
    /// the specified number.
    /// Receives the number to increment the counter and the current time measured in blocks.
    pub fn increment_and_is_allowed(&mut self, number: u16, now: u32) -> bool {
        if now > self.block_range && self.last_reset < (now - self.block_range) {
            self.last_reset = now;
            self.occurrences_counter = 0;
        }
        self.occurrences_counter += number;
        self.occurrences_counter <= self.allowed_occurrences
    }

    /// Fetches the maximum allowed occurences.
    pub fn get_allowed_occurrences(&self) -> u16 {
        self.allowed_occurrences
    }

    /// Fetches the block range.
    pub fn get_block_range(&self) -> u32 {
        self.block_range
    }

    /// Fetches the block height of the last reset.
    pub fn get_last_reset(&self) -> u32 {
        self.last_reset
    }
}
