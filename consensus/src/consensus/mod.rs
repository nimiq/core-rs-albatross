...
use nimiq_network_interface::request::message_handler;
...
use crate::messages::{
    PushAdressNotification, RequestBlock, RequestHead, RequestMacroChain, RequestMissingBlocks,
};
#[cfg(feature = "full")]
use crate::messages::{
...
/// For instance: we might be interested in knowing about transactions, from some specific adress, that are included in a block
#[derive(Clone)]
pub enum RemoteEvent {
    /// Interesting receipts: They belong to an adress that is interesting to us
...
pub struct Consensus<N: Network> {
    ...
    /// This stream is used to communicate remote events to which we are subscribed
    remote_events: BroadcastSender<RemoteEvent>,
...
impl<N: Network> Consensus<N> {
    ...
    pub fn new(
        ...
        // Setup the remote events handler
        let (remote_tx, _rx) = broadcast(256);

        let stream = network.receive_messages::<PushAdressNotification>();
        executor.exec(Box::pin(message_handler(&network, stream, &remote_tx)));
    ...
    /// Subscribe to remote events, i.e: events that are generated by a peer
