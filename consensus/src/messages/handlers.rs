...
#[cfg(feature = "full")]
impl<N: Network> Handle<N, Arc<RwLock<Blockchain>>> for RequestKeccak256HistoryRoot {
    fn handle(
        &self,
        _peer_id: N::PeerId,
        blockchain: &Arc<RwLock<Blockchain>>,
    ) -> Result<ResponseKeccak256HistoryRoot, ResponseKeccak256HistoryRootError> {
        if !Policy::is_macro_block_at(self.block_number) {
            return Err(ResponseKeccak256HistoryRootError::NotAMacroBlock(
                self.block_number,
            ));
        }
        ...
        let block = blockchain
            .chain_store
            .get_block_at(self.block_number, false, None)
            .map_err(|_| ResponseKeccak256HistoryRootError::BlockNotFound(self.block_number))?;

        let root = blockchain
            .history_store
            .get_keccak256_history_root(self.block_number, None)
            .ok_or(ResponseKeccak256HistoryRootError::NotSupported)?;

        Ok(ResponseKeccak256HistoryRoot { root, block })
    }
}
