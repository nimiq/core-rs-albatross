...
async fn two_peers_can_sync_empty_chain() {
    sync_two_peers(0, 0, SyncMode::Light).await
}

#[test(tokio::test)]
async fn two_peers_can_sync_single_batch() {
    sync_two_peers(1, 0, SyncMode::Light).await
}

#[test(tokio::test)]
async fn two_peers_can_sync_two_batches() {
    sync_two_peers(2, 0, SyncMode::Light).await
}
...
async fn two_peers_can_sync_single_epoch() {
    sync_two_peers(Policy::batches_per_epoch() as usize, 0, SyncMode::Light).await
}
