mod sync_utils;

use nimiq_primitives::policy::Policy;
use nimiq_test_log::test;

use crate::sync_utils::{
    sync_two_peers, sync_two_peers_across_protocol_upgrade,
    sync_two_peers_across_protocol_upgrades, SyncMode,
};

#[test(tokio::test)]
async fn two_peers_can_sync_empty_chain() {
    sync_two_peers(0, 1, SyncMode::Full).await
}

#[test(tokio::test)]
async fn two_peers_can_sync_single_batch() {
    sync_two_peers(1, 1, SyncMode::Full).await
}

#[test(tokio::test)]
async fn two_peers_can_sync_two_batches() {
    sync_two_peers(2, 1, SyncMode::Full).await
}

#[test(tokio::test)]
async fn two_peers_can_sync_epoch_minus_batch() {
    sync_two_peers(
        (Policy::batches_per_epoch() - 1) as usize,
        1,
        SyncMode::Full,
    )
    .await
}

#[test(tokio::test)]
async fn two_peers_can_sync_epoch_plus_batch() {
    sync_two_peers(
        (Policy::batches_per_epoch() + 1) as usize,
        1,
        SyncMode::Full,
    )
    .await
}

#[test(tokio::test)]
async fn two_peers_can_sync_epoch_plus_two_batches() {
    sync_two_peers(
        (Policy::batches_per_epoch() + 2) as usize,
        1,
        SyncMode::Full,
    )
    .await
}

#[test(tokio::test)]
async fn two_peers_can_sync_single_epoch() {
    sync_two_peers(Policy::batches_per_epoch() as usize, 1, SyncMode::Full).await
}

#[test(tokio::test)]
async fn two_peers_can_sync_two_epochs() {
    sync_two_peers(
        (Policy::batches_per_epoch() * 2) as usize,
        1,
        SyncMode::Full,
    )
    .await
}

#[test(tokio::test)]
async fn protocol_upgrade_event_is_emitted_during_full_sync() {
    sync_two_peers_across_protocol_upgrade(SyncMode::Full).await
}

#[test(tokio::test)]
async fn protocol_upgrade_events_are_emitted_during_full_sync_across_multiple_upgrades() {
    sync_two_peers_across_protocol_upgrades(SyncMode::Full, 2).await
}
