...
use nimiq_network_interface::peer::{CloseReason, Peer, RequestResponse, SendError};
...
impl Network for MockNetwork {
    ...
    async fn subscribe<T>(&self, _topic: &T) -> Result<Box<dyn Stream<Item = (T::Item, usize)> + Send + Unpin>, Self::Error>
    where
        T: Topic + Sync,
    {
        unimplemented!()
