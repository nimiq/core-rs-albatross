...
fn criterion_benchmark(c: &mut Criterion) {
    c.bench(
        "from_str",
        Benchmark::new("FixedUnsigned", |b| {
            b.iter(|| FixedUnsigned10::from_str(NUM_1).unwrap())
        })
        .with_function("BigDecimal", |b| {
            b.iter(|| BigDecimal::from_str(NUM_1).unwrap())
        })
        .with_function("BigDecimal::with_prec", |b| {
            b.iter(|| BigDecimal::from_str(NUM_1).unwrap().with_prec(PRECISION))
        }),
    );
    c.bench(
        "to_str",
    ...
    c.bench(
        ...
        Benchmark::new("FixedUnsigned", |b| b.iter(|| &*FIXED_1 + &*FIXED_2))
            ...
            .with_function("BigDecimal::with_prec", |b| {
                b.iter(|| (&*BIGDECIMAL_1 + &*BIGDECIMAL_2).with_prec(PRECISION))
            }),
    );
    c.bench(
        "sub",
