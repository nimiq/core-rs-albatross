...
fn it_can_register_notify_deregister() {
    ...
    let handle1 = notifier.register(move |e: &u32| {
        *event1_rc2.write().unwrap() = *e;
    });
    assert_eq!(*event1_rc1.read().unwrap(), 0);

    let event2_rc1 = Arc::new(RwLock::new(0));
    let event2_rc2 = event2_rc1.clone();
    let handle2 = notifier.register(move |e: &u32| {
        *event2_rc2.write().unwrap() = *e;
    });
    ...
    let handle3 = notifier.register(move |e: &u32| {
        *event3_rc2.write().unwrap() = *e;
    });
    assert_eq!(*event3_rc1.read().unwrap(), 0);

    let event4_rc1 = Arc::new(RwLock::new(0));
    let event4_rc2 = event4_rc1.clone();
    let _handle4 = notifier.register(move |e: &u32| {
        *event4_rc2.write().unwrap() = *e;
    });
    ...
    let handle5 = notifier.register(move |e: &u32| {
        *event5_rc2.write().unwrap() = *e;
    });
...
fn it_can_pass_through_notifications() {
    ...
    let event2_rc1 = Arc::new(RwLock::new(0));
    let event2_rc2 = event2_rc1.clone();
    notifier.register(move |e: u32| *event2_rc2.write().unwrap() = e);
    assert_eq!(*event2_rc1.read().unwrap(), 0);
